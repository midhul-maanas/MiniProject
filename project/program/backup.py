...
#                 current_session_time = current_time - activity_data[proc_name]['total_time']
...
#                     prev_completed_time = activity_data[proc_name].get('prev_time', 0)
                
#                 # ✅ Total time = Previous sessions + Current session
#                 total_time = prev_completed_time
        ...
        try:
            ...
            for proc in psutil.process_iter(['pid','name','cpu_percent']):
                try:
                    ...
                    #     }
                    if proc_name in activity_data:
                        if (activity_data[proc_name]['status'] == 'completed'):
                            activity_data[proc_name]['status'] = 'running'

                        delta = current_time - activity_data[proc_name]['last_update']   
                        activity_data[proc_name]['prev_time'] = activity_data[proc_name]['total_time']
                        activity_data[proc_name]['total_time'] += delta
                        activity_data[proc_name]['cpu_usage'] += cpu
                        activity_data[proc_name]['last_update'] = current_time
                        current_processes[pid]['cpu'] = activity_data[proc_name]['cpu_usage'] + cpu
                    else:
                        activity_data[proc_name]={
                            'total_time': 0,
                            'cpu_usage': cpu,
            ...
            for pid in ended_pids:
                proc_name = tracked_processes[pid]['name'] 
                cpu = tracked_processes[pid]['cpu']
            
                activity_data[proc_name]['status'] = 'completed'
                activity_data[proc_name]['last_update'] = time.time()
                tot = activity_data[proc_name]["total_time"]
                print(f"✅ Completed: {proc_name}")
                print(f"   Total time: {tot/60:.1f} min")
                
            # Update tracked processes    
            tracked_processes.clear()  #needed????
...
def add_activity():
    ...
    activity_data[identifier]['last_update'] = time.time()
